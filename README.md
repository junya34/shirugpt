...
> 利用者ごとの使用量ログ・週次の使用制限にも対応済みです。Cloud Run 等への移行、
...
管理者ページの「使用制限」タブから、利用者ごとに**週間の Gemini トークン代
（USD）の上限**を設定できます（個別編集・全員への一括適用の両方に対応）。
未設定の利用者には既定値（`$1.00`、`src/config.py` の
`DEFAULT_WEEKLY_LIMIT_USD`）が適用されます。上限に達すると、その回答は
最後まで完了した上で、次の質問からブロックされます（月曜0:00 JST に
...
| `BQ_LIMIT_TABLE` | `user_limits` | 週次使用制限のテーブル名（`BQ_LOG_DATASET` と同じデータセット内） |
...
└── src/
    ...
    ├── usage_log.py    利用者ごとの使用量ログ・週次使用制限（Gemini 境界の外）
...
- Streamlit Cloud に限定公開する運用を前提にしています。アプリ独自の権限管理
  ...
  展開はまだ想定していません。
- 週次の使用制限は、同一アカウントを複数タブ・複数デバイスで**同時に**使った
