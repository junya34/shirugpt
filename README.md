...
# 同僚に URL を共有する（Streamlit Community Cloud）

GitHub Pages は静的サイトのみのため、Python バックエンドが動き続けるこのアプリは
直接ホストできません。代わりに **Streamlit Community Cloud** を使います。
このリポジトリと直接連携してデプロイでき、公開 URL が発行されます。

> **利用者認証はありません。** URL を知っていれば誰でも BigQuery にクエリを
> 投げられます（allowlist・SQL ガードは効きますが、アクセスできる人の制限は
> ありません）。この前提で共有することを選んだ場合の手順です。

## 手順

1. https://share.streamlit.io を開き、GitHub アカウントでサインインする
2. 「New app」→ リポジトリ `junya34/shirugpt` / ブランチ `main` / メインファイル
   `app.py` を指定してデプロイする
3. デプロイ後、アプリの「Settings」→「Secrets」に以下を貼り付ける
   （`<...>` の部分を実際の値に置き換える）

   ```toml
   GCP_PROJECT_ID = "<プロジェクトID>"
   BQ_ALLOWED_DATASETS = "202506"

   GCP_SERVICE_ACCOUNT_JSON = '''
   <サービスアカウントキーの JSON を貼り付ける>
   '''
   ```

   **`'''`（シングルクォート 3 つ）で必ず囲むこと。** `"""`（ダブルクォート）
   にすると、TOML が JSON 内の `\n` を実際の改行に変換してしまい、
   `private_key` を含む JSON が壊れます（実機で確認済みの落とし穴）。

4. 保存すると自動的に再起動し、発行された URL がそのまま共有先になる

この Secrets の貼り付けだけは、実際の鍵の中身を扱う操作のため**ご自身で**
行ってください。

## 仕組み

[app.py](app.py) は起動時に `bridge_streamlit_secrets()`（[src/config.py](src/config.py)）
を呼びます。ローカル開発では `secrets.toml` が存在せず例外になるため何もせず、
既存の `.env` ベースの読み込みにフォールバックします。Streamlit Cloud 上では
`st.secrets` の値を環境変数へ橋渡しし、`GCP_SERVICE_ACCOUNT_JSON` は一時ファイルに
書き出して `GOOGLE_APPLICATION_CREDENTIALS` に設定します。**ローカルの `.env` や
`credentials/` の運用には影響しません。**

## GCP 側で必要なこと

Secrets に貼るサービスアカウントには、[セットアップ 2 章](#2-gcp-側の準備)と同じ
最小権限（`bigquery.dataViewer` / `bigquery.jobUser` / `aiplatform.user`）を
付与したものを新規に発行することを推奨します。ローカル開発用の鍵とは分けて
管理すると、片方が漏れてももう片方への影響を避けられます。

## 別の選択肢: Cloud Run

GCP 内で完結させたい場合や、将来的に IAP で認証を足したい場合は
[Dockerfile](Dockerfile) を使った Cloud Run デプロイも用意してあります。
Cloud Run はコンテナに焼き込まれた鍵ではなく、実行時サービスアカウントによる
ADC を使うため鍵ファイルそのものが不要になり、Streamlit Cloud より鍵の管理が
安全になります。ただし構築には Docker と `gcloud` の追加設定が要ります。
